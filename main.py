        ...
        with open(LIBRARY_FILE, "r") as file:
            ...
            for line in file:
                ...
                if line.startswith("Book Title = "):
                    book["title"] = line.replace("Book Title = ", "")
                elif line.startswith("Author = "):
                    book["author"] = line.replace("Author = ", "")
                elif line.startswith("Year = "):
                    book["year"] = int(line.replace("Year = ", ""))
                elif line.startswith("Genre = "):
                    book["genre"] = line.replace("Genre = ", "")
                elif line.startswith("Read Status = "):
                    book["read_status"] = line.replace("Read Status = ", "") == "Read"
                    library.append(book)
                    book = {}  # Reset book dictionary for next entry
            return library
    return []
...
def save_library(library):
    with open(LIBRARY_FILE, "w") as file:
        for book in library:
            file.write(f"Book Title = {book['title']}\n")
            file.write(f"Author = {book['author']}\n")
            file.write(f"Year = {book['year']}\n")
            file.write(f"Genre = {book['genre']}\n")
            file.write(f"Read Status = {'Read' if book['read_status'] else 'Unread'}\n\n")
